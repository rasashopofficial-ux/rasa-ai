...
export default function RasaAI() {
  ...
  var generate = useCallback(async function() {
    ...
    try {
      ...
      if (tool === "image") {
        userPrompt = 'Generate ultra viral premium image content for ' + pName + ' ' + format + '. Tone: ' + tone + '. Brief: ' + prompt + '. Return JSON with these exact keys: imagePrompt (write an extremely detailed 300+ word photorealistic prompt using Flux Pro / Midjourney v6 / DALLE-3 style. Include: exact subject description, facial features if person, outfit details, exact location setting, time of day, weather, lighting setup (golden hour/studio/neon/etc), camera (Sony A7R5/Canon R5/Hasselblad), lens (85mm f/1.2 / 35mm f/1.4), aperture and depth of field, color grading (Kodak Portra 400/Fuji Velvia/cinematic LUT), mood and atmosphere, background details, foreground elements, texture details, visual style reference (editorial/commercial/lifestyle/cinematic), 8K ultra sharp, photorealistic, professional photography), negativePrompt (ugly, blurry, low quality, watermark, text overlay, cartoon, CGI, oversaturated, distorted face, extra limbs, deformed, amateur), caption (viral platform caption with strategic emoji placement, curiosity gap opening, storytelling element), captionV2 (alternative caption with bold statement), captionV3 (short punchy under 100 chars), hook (neurologically optimized 3 second scroll stopping hook using open loop technique), hookV2 (controversy or bold claim hook), hookV3 (relatable situation hook), hashtags (array of 15 targeted tags mixing niche and broad), viralHashtags (array of 5 most viral trending tags right now), cta (psychology driven CTA that drives saves and shares), bestTime (exact day time and timezone for peak engagement), viralityScore (number between 75 and 95), viralityReason (specific algorithm reason this will perform well), algorithmTip1 (specific platform algorithm tip), algorithmTip2, algorithmTip3, algorithmTip4, algorithmTip5, postingStrategy (detailed what to do in first 60 minutes after posting to maximize algorithm boost), competitorInsight (what top viral creators do differently in this exact niche), colorPalette (array of 3 hex colors that match the visual))';
      } else if (tool === "video") {
      ...
      if (tool === "image" && data.imagePrompt) {
        data.imageUrl = "https://image.pollinations.ai/prompt/" + encodeURIComponent(data.imagePrompt.slice(0, 500)) + "?width=1080&height=1080&nologo=true&model=flux&enhance=true&seed=" + Date.now();
      }
