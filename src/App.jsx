...
const VIDEO_MODELS = [
  {id:"sora2_free",name:"🎬 Sora 2 (Free)",plan:"Free"},
  {id:"sora2_paid",name:"🎬 Sora 2",plan:"Starter"},
  {id:"kling16",name:"✨ Kling 1.6 Pro",plan:"Creator"},
  {id:"runway",name:"🚀 Runway Gen-3",plan:"Pro"},
];

const TONES = ["Viral","Luxury","Funny","Educational","Inspiring","Edgy"];

const DEFAULT_PLANS = [
  {id:"starter",name:"Starter",price:199,credits:100,period:"mo",color:"#a855f7",features:["100 credits/month","All AI tools","All 6 platforms","Sora 2 video","Email support"],videoModel:"sora2_paid"},
  {id:"creator",name:"Creator",price:499,credits:300,period:"mo",color:"#D946EF",popular:true,features:["300 credits/month","All Starter features","Priority generation","4K Image export","Priority support"],videoModel:"kling16"},
  {id:"pro",name:"Pro",price:999,credits:1000,period:"mo",color:"#22D3EE",features:["1000 credits/month","All Creator features","Early access models","API access (soon)","Dedicated support"],videoModel:"runway"},
  {id:"impulse",name:"Impulse Pack",price:119,credits:50,period:"6 days",color:"#F59E0B",features:["50 credits","One-time purchase","Expires in 6 days","No subscription","Perfect viral push"],videoModel:"sora2_free"},
];
...
export default function App(){
  ...
  function handleSignout(){
    setUser(null);setPage("landing");setEmail("");setPass("");setName("");
  }

  const generate=useCallback(async()=>{
    if(!prompt.trim()&&tool!=="img2vid"&&tool!=="aud2vid"){setError("Please enter a description");return;}
    if(credits<=0){setError("No credits!");return;}
    if(tool==="video"&&videoModel==="sora2_free"&&sora2Used>=2&&user?.plan==="Free"){setError("Sora 2 free limit reached!");return;}
    ...
    try{
      let userPrompt="";
      if(tool==="image"){userPrompt=`Generate viral image content for ${pName} ${format}. Tone: ${tone}. Brief: ${prompt}. Return JSON: imagePrompt (200 word photorealistic prompt), negativePrompt, caption, hashtags (array).`;}
      else if(tool==="video"){userPrompt=`Generate viral video script for ${pName} ${format} optimized for ${selModel}. Tone: ${tone}. Brief: ${prompt}. Return JSON: videoPrompt (200 word cinematic prompt), script, hook, caption, hashtags.`;}
      else if(tool==="img2vid"){userPrompt=`Generate image to video animation for ${pName}. File: ${uploadedFile?.name||"image"}. Tone: ${tone}. Instructions: ${prompt||"Animate naturally"}. Return JSON: animationPrompt, caption, hashtags.`;}
      else if(tool==="aud2vid"){userPrompt=`Generate audio to video visual for ${pName}. Audio: ${uploadedAudio?.name||"audio"}. Tone: ${tone}. Brief: ${prompt||"Match audio mood"}. Return JSON: visualPrompt, caption, hashtags.`;}
      else if(tool==="caption"){userPrompt=`Generate viral captions for ${pName} ${format}. Tone: ${tone}. Content: ${prompt}. Return JSON: caption1, caption2, caption3, hashtags.`;}
      else if(tool==="resize"){userPrompt=`Generate resize guide for all platforms. Original: ${format} for ${pName}. Content: ${prompt}. Return JSON with dimensions.`;}
